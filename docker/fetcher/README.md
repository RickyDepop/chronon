...
| OTEL_RESOURCE_ATTRIBUTES          | Comma-separated `key=value` pairs appended to OTLP resource attributes (e.g., `deployment.environment=prod,team=ml`). Attached to metrics emitted by both the Chronon library (OTel SDK) and the Vert.x HTTP server (Micrometer). Follows the [OTel spec](https://opentelemetry.io/docs/specs/otel/resource/sdk/#specifying-resource-information-via-an-environment-variable). | ``                      |
| **Monitoring**                    
